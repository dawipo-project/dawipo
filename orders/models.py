...
class Order(models.Model):
	...
	shipping = models.DecimalField(max_digits=10, decimal_places=2, null=True, blank=True, default=0.0)
	observations = models.TextField(null=True, blank=True)
	payment_method = models.ForeignKey(PaymentMethod, 
		related_name='order_payment', on_delete=models.CASCADE, null=True, blank=True)
	discount = models.DecimalField(max_digits=4, decimal_places=2, null=True, blank=True, default=0.0)
	...
	def get_total_discount(self):
		if self.discount and self.discount > 0:
			return round(self.get_cost() * (self.discount / 100), 2)
		else:
			return 0

	def get_total_cost(self):
		if self.shipping and self.shipping is not 0 and self.tax:
			return round((((self.get_cost() + self.get_total_tax()) - self.get_total_discount()) + self.shipping), 2)
		elif not self.shipping and self.tax:
			return round(((self.get_cost() + self.get_total_tax()) - self.get_total_discount()), 2)
		elif not self.shipping and not self.tax:
			return round(self.get_cost() - self.get_total_discount(), 2)
...
class OrderItem(models.Model):
	...
	def get_price(self):
		if self.order.tax and self.tax is not 0:
			return round(self.price - ((self.price / 100) * self.tax), 2)
		else:
			return self.price
